...
lemma gd_convex_potential_step {f : E → ℝ} {β : ℝ} (hβ : 0 < β)
  ...
  have hexp : ‖w' - z‖ ^ 2 = ‖w - z‖ ^ 2
    ...
    calc
      ...
      _ = ‖w - z‖ ^ 2 - 2 * inner ℝ (w - z) (β⁻¹ • gradient f w) + ‖β⁻¹ • gradient f w‖ ^ 2 := by
        rw [norm_sub_sq_real]
      _ = ‖w - z‖ ^ 2 - 2 * inner ℝ (w - z) (β⁻¹ • gradient f w) +
            (β⁻¹)^2 * ‖gradient f w‖ ^ 2 := by
        ...
        rw [hnorm]
      _ = ‖w - z‖ ^ 2 - 2 * (β⁻¹ * inner ℝ (w - z) (gradient f w)) +
            (β⁻¹)^2 * ‖gradient f w‖ ^ 2 := by
        simp [inner_smul_right]
      _ = ‖w - z‖ ^ 2 - 2 * β⁻¹ * inner ℝ (gradient f w) (w - z) +
            (β⁻¹)^2 * ‖gradient f w‖ ^ 2 := by
  ...
  have hneg : -2 * β⁻¹ * inner ℝ (gradient f w) (w - z) ≤ 2 * β⁻¹ * (f z - f w) := by
    ...
    nlinarith
  have hsum : -2 * β⁻¹ * inner ℝ (gradient f w) (w - z) + (β⁻¹)^2 * ‖gradient f w‖ ^ 2 ≤
      2 * β⁻¹ * (f z - f w') := by
    nlinarith
  nlinarith
...
theorem gd_convex_convergence_simplified {f : E → ℝ} {β : ℝ} (hβ : 0 < β)
  ...
  have h_sub : ‖w₀ - z‖ ^ 2 - ‖gdIterate f (fun _ => β⁻¹) w₀ t - z‖ ^ 2 ≤ ‖w₀ - z‖ ^ 2 := by
    nlinarith
