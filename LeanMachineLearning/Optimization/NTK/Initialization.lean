...
Theorem 2 / Step 2 (Strong Law of Large Numbers for the Covariance Tensor), and Theorem 2.3 /
Theorem 3 (Finite-Dimensional NNGP Limit at Initialization) for neural networks.
...
* `NTK.isGaussianProcess_exact_conditional_output` : Definition 2.2 exact conditional
  Gaussian process.
...
* `NTK.sum_sum_mul_limitingCovariance_eq_integral_sq` : Theorem 2.3 Step 4 expectation-of-square
  identity verifying condition (i) of Definition 2.2.
* `NTK.sum_sum_mul_limitingCovariance_nonneg` : nonnegative limiting kernel quadratic form.
