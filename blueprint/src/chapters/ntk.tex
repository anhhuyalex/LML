...
For any $x \in \mathbb{R}^d$ with $\|x\| \le 1$ and any $W, V \in \mathbb{R}^{m \times d}$,
\[
  |f(x; W) - f_0(x; V)| \le \frac{\beta}{2\sqrt{m}} \|W - V\|_{\mathrm{F}}^2.
\]
\end{proposition}
...
\]
Summing over neurons, using $|a_j| \le 1$ and $|\langle w_j - v_j, x\rangle| \le \|w_j - v_j\| \cdot \|x\| \le \|w_j - v_j\|$:
\begin{align*}
  |f(x;W) - f_0(x;V)|
  &= \frac{1}{\sqrt{m}}\left|\sum_{j=1}^m a_j
      \bigl[\sigma(w_j^\top x) - \sigma(v_j^\top x) - \sigma'(v_j^\top x)(w_j-v_j)^\top x\bigr]
     \right| \\
  &\le \frac{1}{\sqrt{m}} \sum_{j=1}^m \frac{\beta(w_j^\top x - v_j^\top x)^2}{2} \\
  &\le \frac{\beta}{2\sqrt{m}} \sum_{j=1}^m \|w_j - v_j\|^2
   = \frac{\beta}{2\sqrt{m}} \|W - V\|_{\mathrm{F}}^2. \qedhere
\end{align*}
\end{proof}

\begin{remark}
Proposition~\ref{prop:ntk:smoothLinearization} does not require Gaussian initialization:
it holds for any $W, V$.
The key consequence is that as $m \to \infty$, the linearization error $|f - f_0|$ decays
as $\mathcal{O}(1/\sqrt{m})$ whenever $\|W - V\|_{\mathrm{F}} = \mathcal{O}(1)$.
\end{remark}
...
\[
  ...
  \frac{6B^{4/3} + 2B\ln(1/\delta)^{1/4}}{m^{1/6}}.
\]
\end{lemma}
...
\begin{definition}[Empirical NTK]\label{def:ntk:empiricalNTK}
  \uses{def:ntk:shallowNetwork, def:ntk:gaussianInit}
...
\[
  ...
  = x^\top x' \cdot \frac{1}{m} \sum_{j=1}^{m}
...
$\sigma'(w_{0,j}^\top x)\,\sigma'(w_{0,j}^\top x')$ are i.i.d.\ and integrable
(bounded for the ReLU).
The strong law of large numbers gives the almost sure convergence of their empirical
...
\begin{proof}
  \uses{def:ntk:rkhs, def:ntk:limitingNTK, prop:ntk:reluNTK}
Consider the bijection $\phi : \mathcal{X} \to U := \{u \in \mathbb{R}^{d-1} : \|u\|^2 \le 1/2\}$
that drops the $d$-th coordinate (which equals $1/\sqrt{2}$ on $\mathcal{X}$).
Under $\phi$, the kernel becomes $k(\phi^{-1}(u), \phi^{-1}(u')) = \tilde{k}(u, u')$, where
\[
  \tilde{k}(u, u') = \tilde{f}(u^\top u'), \qquad
  \tilde{f}(z) := \frac{z + 1/2}{2} - \frac{(z+1/2)\,\arccos(z+1/2)}{2\pi}.
\]
The Maclaurin series of $\arccos$ shows that every coefficient of the power series of
$\tilde{f}$ is strictly positive.
...
Hence $\tilde{k}$ is a universal kernel over $U$, and $\mathcal{H}$ is a universal
