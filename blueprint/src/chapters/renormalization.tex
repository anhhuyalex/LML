...
\lean{NeuralNetwork.posHomogeneous_iff_eq_leakyRelu}
...
\end{align*}
The hidden index type is existentially stored by the constructor, so ill-shaped layer composition
is unrepresentable.  The first implementation should use arbitrary finite index types rather than
hard-coding \texttt{Fin n}; a \texttt{Fin}-width constructor supplies the familiar presentation.
\end{definition}
...
\]
At a fixed dataset this evaluates to the Dirac measure at the computed output.  Integrating this
kernel against the parameter law recovers Definition~\ref{def:renorm:outputLaw} through
...
$C_b,C_W\in\mathbb R_{\ge0}$ and require $I$ to be nonempty.  The variance of every weight is
$C_W/|I|$ and the variance of every bias is $C_b$.
\end{definition}
...
\]
Take their product and map the resulting pair to \texttt{DenseLayer}.  The construction uses
finite \texttt{Measure.pi}; no \texttt{Fintype} instance is requested for an infinite index type,
and no blanket \texttt{IsGaussian (Measure.pi ...)} instance is assumed.
\end{definition}
...
There are two concrete errors in the state from which this plan starts.
...
\begin{enumerate}
  \item every external declaration used by the existing Chapter~1 implementation has been checked
    in its pinned import context; every new Chapter~2 candidate has been source-searched, and
    implementation begins with a fresh exact-import batch-\texttt{\#check} audit before any of
    those candidates is used in code;
  ...
  \item false or underspecified source claims are corrected before proof work: threshold is not
    called continuous, zero-variance Gaussians are measures rather than densities, the Fourier
    delta display is not asserted as a Lebesgue integral, and the CLT claim is deferred until a
    metric and quantitative assumptions are chosen;
...
criterion, an exact-import \texttt{lake env lean} check, and a final full build.  If the proposed
typed MLP representation produces the dependent-type or definitional-equality problems warned
about in Sections~13.1 and~14.17, Phase~N2 explicitly stops and substitutes a scratch-verified
encoding before downstream code is written.  Thus the plan does not use ``elegance'' as a reason
to ignore elaboration evidence.
