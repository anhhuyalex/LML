...
finite Gaussian amplitude, whereas taking depth to infinity at fixed width diverges.  A fully
literal iterated-limit equality is stated in \texttt{ENNReal}; the real theorem records the two
filter limits separately.
\end{corollary}
...
\begin{enumerate}
  ...
  \item If the weak-limit theorem temporarily remains \texttt{sorry}, its docstring must contain
    the informal coupling/Slutsky proof and a precise source as specified above.  No phase is
    accepted as mergeable while such a placeholder remains.
\end{enumerate}
